...
object LlmClient {
    ...
    fun chat(
        ...
        log.warn("AiCode: 发起 chat 请求 $url (model=${provider.model}, stream=$stream, 请求超时=${requestTimeout.toSeconds()}s)")
        log.warn("AiCode: 本次调用 LLM —— provider=${provider.name}, model=${provider.model}")
        ...
        val response: HttpResponse<InputStream> = try {
            ...
            future.cancel(true)
            log.warn("AiCode: chat 请求超时（${System.currentTimeMillis() - start}ms）: $url", e)
        ...
        log.warn("AiCode: chat 响应 HTTP ${response.statusCode()}（${System.currentTimeMillis() - start}ms）")

        if (response.statusCode() !in 200..299) {
            val errBody = runCatching { response.body().reader(StandardCharsets.UTF_8).readText() }
                .getOrDefault("").take(500)
            response.body().close()
            log.warn("AiCode: chat 请求失败 HTTP ${response.statusCode()}：$errBody")
        ...
        log.warn("AiCode: chat 完成（共 ${System.currentTimeMillis() - start}ms，回复 ${result.length} 字符）")
        return result
    }

    /** 连通性测试：发一个极小的请求，返回模型回复（或抛异常）。超时较短，快速给出成败。 */
    fun testConnection(provider: ProviderConfig): String {
        log.warn("AiCode: 测试连接 ${provider.name} (${provider.baseUrl}) model=${provider.model}")
    ...
    fun listModels(provider: ProviderConfig): List<String> {
        ...
        provider.extraHeaders.forEach { (k, v) ->
            if (k.isNotBlank()) runCatching { builder.header(k.trim(), v) }
                .onFailure { log.warn("AiCode: 忽略非法请求头 $k", it) }
        }
        val start = System.currentTimeMillis()
        log.warn("AiCode: 获取模型列表 $url")
        ...
        val response: HttpResponse<String> = try {
            ...
            future.cancel(true)
            log.warn("AiCode: 获取模型列表超时（${System.currentTimeMillis() - start}ms）: $url", e)
        ...
        log.warn("AiCode: 模型列表响应 HTTP ${response.statusCode()}（${System.currentTimeMillis() - start}ms）")

        if (response.statusCode() !in 200..299) {
            val errBody = response.body().take(500)
            log.warn("AiCode: 获取模型列表失败 HTTP ${response.statusCode()}：$errBody")
        ...
        log.warn("AiCode: 获取模型列表成功，共 ${out.size} 个模型（${System.currentTimeMillis() - start}ms）")
        return out.toList()
    }
