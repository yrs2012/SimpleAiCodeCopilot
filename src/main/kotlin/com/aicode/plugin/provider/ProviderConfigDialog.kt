...
class ProviderConfigDialog(existing: ProviderConfig?) : DialogWrapper(null, true) {
    ...
    private fun fetchModels() {
        ...
        if (fetchRunning.get()) {
            ...
            log.warn("AiCode: Auto Add Models 被用户取消")
        ...
        log.warn("AiCode: Auto Add Models 开始（provider=$name, baseUrl=$baseUrl）")
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            log.warn("AiCode: Auto Add Models 网络调用结束（${elapsed}ms，成功=${outcome.isSuccess}）")
            
            if (outcome.isFailure) {
                log.warn("AiCode: Auto Add Models 失败", outcome.exceptionOrNull())
            }
            ...
            javax.swing.Timer(100) { e ->
                ...
                if (fetchCancelled.get()) {
                    log.warn("AiCode: Auto Add Models 已取消")
                ...
                if (outcome.isSuccess) {
                    ...
                    log.warn("AiCode: Auto Add Models 失败：$msg")
...
private fun testConnection() {
    ...
    if (testRunning.get()) {
        ...
        log.warn("AiCode: Test Connection 被用户取消")
    ...
    log.warn("AiCode: Test Connection 开始（provider=${cfg.name}, baseUrl=${cfg.baseUrl}, model=${cfg.model}）")
    ...
    ApplicationManager.getApplication().executeOnPooledThread {
        ...
        val elapsed = System.currentTimeMillis() - start
        log.warn("AiCode: Test Connection 网络调用结束（${elapsed}ms，成功=${outcome.first}）")
        ...
        SwingUtilities.invokeLater {
            ...
            if (testCancelled.get()) {
                log.warn("AiCode: Test Connection 已取消")
                testButton.text = "Test Connection"
            ...
            if (outcome.first) {
                log.warn("AiCode: Test Connection 成功：${outcome.second}")
                testButton.text = "✓ Connection OK"
                testButton.toolTipText = outcome.second
            } else {
                log.warn("AiCode: Test Connection 失败：${outcome.second}")
