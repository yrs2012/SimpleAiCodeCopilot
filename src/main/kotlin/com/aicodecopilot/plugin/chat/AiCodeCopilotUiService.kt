 ...
 * 聊天面板的项目级宿主服务。
 *
 * 工具窗口是惰性创建的；当用户还没打开 AiCodeCopilot 窗口就触发
 * “Send to AiCodeCopilot” 动作时，动作会先把待处理的内容暂存到这里，
 * 等面板创建完成后再应用。
 */
@Service(Service.Level.PROJECT)
class AiCodeCopilotUiService(private val project: Project) {
    ...
    companion object {
        fun getInstance(project: Project): AiCodeCopilotUiService =
            project.getService(AiCodeCopilotUiService::class.java)
    }
}
