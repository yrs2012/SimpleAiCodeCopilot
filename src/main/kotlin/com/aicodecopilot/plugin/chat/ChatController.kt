...
class ChatController(
    ...
    fun send(raw: String) {
        ...
        if (provider0 == null) {
            ui.onError("No LLM provider configured. Open Settings → AiCodeCopilot to add one.")
            return
        }
        ...
        log.info(
            "AiCodeCopilot: 发送消息（provider=${provider.name}, model=${provider.model}, " +
                "baseUrl=${provider.baseUrl}, 历史=${requestMessages.size}条, 上下文=${allEntries.size}项）"
        )
        ...
        Thread({
            try {
                ...
                if (s.messages.lastOrNull()?.content.isNullOrBlank() && cancel.get()) {
                    ...
                    log.info("AiCodeCopilot: 流式回复被用户停止（${System.currentTimeMillis() - start}ms）")
                } else {
                    log.info(
                        "AiCodeCopilot: 流式回复完成（${System.currentTimeMillis() - start}ms，" +
                            "共 ${s.messages.lastOrNull()?.content?.length ?: 0} 字符）"
                    )
                }
            } catch (e: Exception) {
                log.warn("AiCodeCopilot: 请求失败（${System.currentTimeMillis() - start}ms）：${e.message}", e)
    ...
    companion object {
        ...
        const val SYSTEM_PROMPT =
            "You are AiCodeCopilot, an AI coding assistant embedded in Android Studio. " +
